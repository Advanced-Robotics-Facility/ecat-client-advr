        ...
        while (run_loop && client->get_client_status().run_loop){
           ...
           // interpolate motors
            ...
            for (const auto &[esc_id, target] : grippers_set_trj){
                ...
                // Clamp motor ref
                if(ctrl_mode == iit::advr::Gains_Type_POSITION){
                    if(gripper_min_limits.count(esc_id)>0 && gripper_max_limits.count(esc_id)>0){
                        if (gripper_ref < gripper_min_limits[esc_id]) {
                            gripper_ref = gripper_min_limits[esc_id];
                        } else if (gripper_ref > gripper_max_limits[esc_id]) {
                            gripper_ref = gripper_max_limits[esc_id];
                        }
                    }
